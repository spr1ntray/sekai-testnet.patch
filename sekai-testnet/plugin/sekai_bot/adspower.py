...
def find_duplicate_profiles(pairs: list[tuple[str, str]]) -> list[list[str]]:
    ...
    for account_id, profile_id in pairs:
        ...
        if not pid:
            continue
        buckets.setdefault(pid.lower(), []).append(account_id)
...
def _start_params(profile_id: str) -> dict[str, Any]:
    pid = normalize_profile_id(profile_id)
    params: dict[str, Any] = {"open_tabs": 0, "ip_tab": 0, "headless": 0}
    if pid.isdigit():
        params["serial_number"] = pid
    else:
        params["user_id"] = pid
    return params
...
class AdsPowerClient:
    ...
    def start_or_attach(self, profile_id: str) -> AdsPowerSession:
        pid = normalize_profile_id(profile_id)
        if not pid:
            raise AdsPowerError("profile_missing", "Пустой AdsPower profile ID")
        return AdsPowerSession(ws_url=self._start(pid), started_by_us=True)

    def stop(self, profile_id: str) -> None:
        """Always ask AdsPower to close the profile window."""
        try:
            self._get("/api/v1/browser/stop", _start_params(profile_id))
        except Exception:
            return
    ...
    def _api_code(self, payload: dict[str, Any]) -> int:
    ...
    def _start(self, profile_id: str) -> str:
        ...
        for attempt in range(1, _START_MAX_RETRIES + 1):
            ...
            last = msg
            transient = any(token in msg.lower() for token in ("too many", "rate", "timeout", "busy", "connection"))
            if transient and attempt < _START_MAX_RETRIES:
                time.sleep(min(8.0, 1.4 * attempt + random.uniform(0.3, 1.1)))
                continue
            raise AdsPowerError("adspower_start_failed", f"AdsPower start: {msg}")
        raise AdsPowerError("adspower_start_failed", f"AdsPower start: {last}")
    ...
    def _get(self, path: str, params: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if self._api_key and self._api_key in msg:
            payload["msg"] = msg.replace(self._api_key, "[REDACTED]")
        return payload
