...
`run_adspower_cleanup_accounts` не дублирует эти секреты. Для восстановления незавершённой очистки таблица хранит только run ID, canonical public account ID и timestamp. AdsPower profile ID остаётся внутри зашифрованного account payload, API key — внутри `vault_secrets`; host расшифровывает их только после unlock и только когда следующий AdsPower-run получил очередь для обязательного reconciliation.
...
| `permissions.network`, `browser`, `financial_risk` | Являются декларативными метаданными; AdsPower grants требуют `browser=true` | Не образуют принудительный allowlist или запрет системных вызовов |
| `permissions.local_services` | Canonical service `adspower` включает durable FIFO, singleton lease, два preflight snapshot и host cleanup exact выбранных profiles: Active блокирует spawn, managed IDs закрываются до 3 секунд стабильного Inactive, а незавершённый cleanup scope переживает terminal/restart | Не запрещает прямые системные вызовы и не координирует другую копию AdsPower, другой data directory или стороннее обращение после preflight |
...
AdsPower имеет дополнительную host-owned границу, не зависящую от `risk`. При admission Hub сохраняет durable claim с монотонной последовательностью, а перед выдачей секретов и spawn пытается получить единственный service lease `adspower`. Claim живёт только пока run активен и удаляется его terminal commit, поэтому рабочая FIFO-таблица не превращается в бесконечную историю. Более ранний активный claim имеет приоритет, поэтому через один Hub одновременно работает не больше одного AdsPower-софта. Это правило охватывает read/parsing, разные аккаунты, отдельные старты и элементы разных `parallel`-пачек. Пока run имеет stage `waiting_for_adspower`, он не удерживает subprocess slot, не получает расшифрованный Vault payload и не создаёт plugin process; после освобождения сервиса старт происходит автоматически в FIFO-порядке.

После получения gate host выполняет два независимо rate-limited Local API snapshot exact profile scope выбранных аккаунтов и выданных direct parents до plugin spawn. Ни один ID не должен быть Active. Уже Active профиль не становится owned, не будет остановлен Hub и блокирует spawn понятной ошибкой с просьбой закрыть его вручную. Если Local API недоступен уже здесь, run безопасно завершается до subprocess/side effect и освобождает gate: cleanup ownership ещё не возникло. После успешного preflight bootstrap создаётся без context; exact public account IDs фиксируются в durable cleanup scope до передачи context и первого возможного browser side effect. Profile IDs и API key в эту запись не попадают; посторонние profile IDs Hub не закрывает.

После containment plugin process Hub сам идемпотентно посылает stop только для этого cleanup-set и в рабочей сессии не завершает run, не снимает AdsPower lease/claim и не пропускает следующего AdsPower-софта, пока весь set не остаётся Inactive не меньше 3 секунд. Недоступность Local API **после spawn**, на cleanup, остаётся fail-closed с понятным пользовательским stage и повторными bounded проверками. В отличие от безопасной pre-spawn ошибки, этот API outage не позволяет освободить gate по TTL; если shutdown всё же terminalize-ит run во время outage, durable scope и pins остаются. Все обращения одного Local API client сериализованы и разнесены минимум на 1,05 секунды. Host events/stages описывают только состояние группы и не сохраняют profile IDs, API key или raw Local API response.

`expires_at` у service lease является heartbeat, а не разрешением украсть ресурс у живого владельца. Даже после долгого сна или задержки heartbeat более поздний claim не получает AdsPower, пока более ранний run остаётся в active status. Штатный terminal commit освобождает singleton lease, account leases, обычные pins и active claim только после доказанного containment plugin process и подтверждённого cleanup-set. При перезапуске Hub сначала проверяет и при необходимости завершает прежнее process tree. Пока процесс ещё жив либо его принадлежность нельзя безопасно доказать, run остаётся recovery-pending и продолжает удерживать AdsPower gate. После containment terminal failure может освободить обычные ресурсы, но durable cleanup rows и их pins переживают его. Следующий AdsPower owner до собственного preflight/spawn расшифровывает exact profiles по этим pinned account IDs, идемпотентно останавливает их, требует минимум 3 секунды стабильного Inactive и только затем удаляет rows/pins. Не запущенный хвост последовательной пачки отменяется по существующему restart-контракту.

Эта сериализация гарантирует порядок только для запусков, проходящих через один экземпляр Hub и один data directory. Она не является блокировкой самого приложения AdsPower: вручную открытый профиль, другая копия AdsPower, другой Hub с отдельной базой или сторонний скрипт находятся вне очереди. Hub не останавливает профиль, который уже был Active при двух preflight snapshot: такой профиль блокирует run до ручного закрытия. После успешного preflight и durable фиксации scope Hub сам закрывает все выбранные этим run профили. Если сторонняя программа откроет такой выбранный профиль уже после snapshot, Hub всё равно остановит его при cleanup; поэтому один profile ID нельзя одновременно использовать вручную и через текущий run. Плагин по-прежнему обязан корректно закрыть свои tabs/driver/workers и не создавать detached descendants.
...
Non-financial мутация внешнего API объявляется как `external_write`, а не `read`. Для неё нет financial acknowledgement, однако runner выдаёт write-lease на время активного запуска. После force stop или падения и доказанного containment run становится `cancelled` либо `failed`, lease освобождается, но это не доказывает отмену внешней операции: HTTP-сервер мог принять запрос до разрыва локального процесса.

При terminal завершении run — в том числе `failed` и `cancelled` — Hub освобождает account/service leases, обычные pins и active AdsPower claim после доказанного containment plugin process. Пока текущий RunManager остаётся рабочим, AdsPower terminal status обычно появляется только после подтверждённого 3-секундного стабильного Inactive всех выбранных managed profile IDs; если proof ещё нет, gate остаётся занятым fail-closed. При shutdown, в том числе на фоне Local API outage, cleanup loop может завершиться без Inactive-proof и run может стать terminal. Это не теряет ownership: `run_adspower_cleanup_accounts` и связанные account pins переживают terminal status, не дают изменить или удалить аккаунт и очищаются только после обязательного exact-profile reconciliation перед spawn следующего AdsPower-run. Active до preflight профиль не запускается и не закрывается Hub. Освобождение AdsPower-gate также не доказывает внешний итог уже отправленного действия. Отдельного подтверждения внешней сверки нет; review не управляет очисткой. Это решение касается только внутренней координации Hub: он по-прежнему не может доказать, была ли транзакция или запись отправлена до сбоя, поэтому оператор обязан проверить explorer/API перед опасным повтором. При запуске новой версии исторические `needs_attention` нормализуются в `failed`; исходные error, events, results и журнал сохраняются как legacy-след.

Для terminal-проблемы оператор может открыть или скачать очищенный общий журнал, а затем закрыть уведомление: run получает `reviewed` и исчезает только из live-attention projection. Review не меняет исходную ошибку, account states, results или events и не выдаёт запуск за успешный. Повторный запуск не зависит от review. Незавершённый AdsPower cleanup тоже не зависит от review: следующий AdsPower-run автоматически завершит его до собственного preflight, а редактирование/удаление pinned аккаунта останется закрытым до этого момента.

Force-stop является отдельной немедленной операцией и запускается одним нажатием без acknowledgement-фразы. Это завершение локального дерева процессов, а не откат внешнего действия: уже принятый внешний запрос может выполниться. После подтверждённого исчезновения дерева run завершается как `cancelled`; Hub освобождает обычные leases/pins и active service claim, а внешний итог остаётся виден в журнале и результатах. Если AdsPower cleanup не был доказан из-за shutdown, его durable rows/pins остаются до автоматического reconciliation.

Queued run, который ещё не получил concurrency slot, ждёт AdsPower или конфликтующий account lease и не создал subprocess, также замечает safe/force cancellation до расшифровки account bundle и spawn, завершается как `cancelled`, после чего его durable claim удаляется, а pins и leases освобождаются. Для такого pre-spawn run cleanup scope ещё не существует. После передачи context действует отдельный инвариант: незавершённый AdsPower cleanup pin намеренно переживает terminal process lifecycle, пока exact profile scope не подтверждён Inactive.
...
В `parallel` AdsPower service lease и write/exclusive account leases выдаются непосредственно перед каждым выполнением. Если AdsPower, scope или account занят, worker освобождает глобальный subprocess slot, остаётся queued и повторяет попытку; поэтому конфликтующий софт ждёт автоматически, а несвязанные задачи используют свободную ёмкость. AdsPower дополнительно соблюдает durable FIFO между отдельными запусками и разными пачками: режим `parallel` не позволяет более позднему AdsPower-софту обойти ранний claim. В `sequential` следующий пункт ждёт terminal-состояния **всех** более ранних пунктов. Ошибка или отмена раннего run не отменяет остаток пачки, но более поздний run не может обогнать ещё работающий первый. Такой waiter не занимает subprocess slot, не удерживает account/service lease и не получает расшифрованный Vault payload. Это уменьшает время жизни секретов и ресурсов, но не делает последовательные внешние операции транзакцией и не отменяет уже случившийся side effect.

Hub разделяет concurrency трёх уровней. Глобальный `--max-concurrent` ограничивает число plugin subprocess, singleton AdsPower-gate допускает только один AdsPower-софт, а action option `account_concurrency`, если софт его объявил, управляет workers внутри одного subprocess. AdsPower-gate не обнуляет этот внутренний параметр: один допущенный софт может обрабатывать несколько своих профилей в пределах собственного `account_concurrency`, пока все остальные AdsPower-софты ждут. Строгий `/4` требует это поле для account-action; свободный `/5` может положиться на безопасный последовательный default. Core валидирует integer, ограничивает `/5` универсальным пределом `20` и зажимает effective value по числу targets; строгий `/4` дополнительно ограничивает browser schema значением `5`. Для browser-плагина `/5` меньший фактический предел обязан выбрать и проверить автор. Это не rate limiter и не nonce manager: плагин всё равно обязан учитывать provider limits, per-account изоляцию, finite timeouts, cancellation и thread safety своих clients/cache.

При штатном shutdown Hub сначала прекращает admission, сигналит активным plugin processes, ждёт grace period и принудительно завершает оставшиеся до lock Vault. На POSIX Hub очищает всю process group, а bootstrap дополнительно следит за PID родительского runner и убивает собственную process group, если родитель исчез. На Windows Hub удерживает дерево в Job Object с `KILL_ON_JOB_CLOSE`; `taskkill /T /F` остаётся только резервным путём. Пока экземпляр закрывается, новые runs не допускаются. В живом экземпляре AdsPower gate освобождается лишь после process containment и managed-profile cleanup. Если завершение сессии прервало cleanup, durable public-account scope и pins сохраняются: следующий AdsPower run сначала закрывает именно прежние profiles до стабильного Inactive, очищает scope, затем делает собственные два preflight snapshot и только после этого spawn. Намеренно detached-процесс и уже принятый внешним сервисом side effect всё равно нельзя считать доказанно отменёнными.
...
- нет общей координации с другой копией AdsPower, другим data directory или сторонними Local API-клиентами: host контролирует только exact scope, который успешно прошёл preflight и был durable зафиксирован до передачи context; recovery этого scope не блокирует внешних Local API-клиентов;
